...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 22 00:05:48 2022
...
for index in range(targets_all.size):
    if targets_all[index] > 14.3037 and targets_all[index] < 19.0472:
...
fig, ax = plt.subplots()
ax.hist(targets_train, rwidth=0.85, bins=3, color= 'skyblue', label='Training Data')
...
plt.xlabel('Adhesive Free Energy', fontsize=15)
...
flipped_predictors = np.flip(predictors_train, 1)
predictors_train = np.vstack([predictors_train, flipped_predictors])
...
ax.plot(np.arange(np.min(targets_all)-1, 28), np.arange(np.min(targets_all)-1, 28),'k-', linewidth=3 )
